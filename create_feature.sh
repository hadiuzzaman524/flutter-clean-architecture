  ...
  class ${ENTITY_CAMEL}RepositoryImpl implements ${ENTITY_CAMEL}Repository {
    ...
    Future<List<${ENTITY_CAMEL}Entity>> getAll() async {
      final response = await remoteDataSource.fetchData();
      final models = (response as List)
          .map((json) => ${ENTITY_CAMEL}ResponseModel.fromJson(json))
          .toList();
  ...
  class ${ENTITY_CAMEL}PortraitView extends StatelessWidget {
    ...
    Widget build(BuildContext context) {
      return Scaffold(
        ...
        body: Center(child: AppText.bodyMedium("Welcome to the new feature")),
      );
    }
  }
  EOF
  ...
  echo "✅ Full feature '$FEATURE_NAME' generated with auto part update!"

  # ---------------------------
  # Run build_runner automatically
  # ---------------------------
  echo "🔹 Running build_runner to generate Freezed and Retrofit files..."
  if command -v flutter >/dev/null 2>&1; then
    flutter pub get
    flutter pub run build_runner build --delete-conflicting-outputs
  else
    echo "⚠️ Flutter not found, running plain Dart build_runner..."
    dart pub get
    dart run build_runner build --delete-conflicting-outputs
  fi

  echo "🎉 Feature '$FEATURE_NAME' is fully generated and build_runner completed!"
